...
mainPath = 'C:\Users\itzic\Desktop\ASUS backup\4th year\IP\Project without PHP combined\Project\test images 2';
imagePattern = fullfile(mainPath, '*.jpeg');
imageFiles = dir(imagePattern);

for k = 1 : length(imageFiles)
    imageName = imageFiles(k).name;
    imagePath = fullfile(mainPath, imageName);
...
%     mask = bwareaopen(mask, 100);
    ...
    separatedHueChannelColorCodeImage = separatedHsvColorCodeImage(:, :, 1);
    detectedCode = [];
